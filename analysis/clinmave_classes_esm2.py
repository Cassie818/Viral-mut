    ...
    for fold, (train_idx, test_idx) in enumerate(splits, start=1):
        ...
        if score_scaling == "standardized":
            train_scores, test_scores = standardize_train_test(
                train, test, ["esm2_650m_score", "calm_score"]
            )
        else:
            train_scores = train[["esm2_650m_score", "calm_score"]].to_numpy(float)
            test_scores = test[["esm2_650m_score", "calm_score"]].to_numpy(float)
...
def main() -> None:
    ...
    if args.analysis_table is not None:
        ...
        pd.concat(all_folds, ignore_index=True).to_csv(
            args.outdir / "clinmave_esm2_650m_calm_cv_fold_metrics.csv", index=False
        )
        pd.DataFrame(summaries).to_csv(
            args.outdir / "clinmave_esm2_650m_calm_cv_summary.csv", index=False
        )
        pd.concat(all_oof, ignore_index=True).to_csv(
            args.outdir / "clinmave_esm2_650m_calm_oof_predictions.csv.gz",
            index=False,
            compression="gzip",
        )
    ...
    if args.write_analysis_table:
        pd.concat(merged_outputs, ignore_index=True).to_csv(
            args.outdir / "clinmave_esm2_650m_calm_analysis_table.csv", index=False
        )
